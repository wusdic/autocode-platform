...
多项目自动编程平台——基于 Hermes Agent v0.16+（v0.17 已验证兼容）的完整设计方案、操作手册与**可部署实现**。
...
> dev-worker/qa/release 的 terminal 跑在短暂 Docker 容器里。故资源占用远低于"17×"的直觉。

## 设计文档

- **[01-最终设计方案.md](01-最终设计方案.md)**：完整架构设计，包含五条底层原则、三层隔离模型、17 角色权限矩阵、需求双层结构、设计委员会多模型质疑机制、先设计再执行的双层硬拦、90 轮问题处理、风险规避表。
...
```
platform/
  control_plane.py      FastAPI 控制平面（对外唯一入口；操作端点 + 用户面只读端点 + Web UI）
...
> ⚠️ 真正运行需要一台 **Ubuntu 22.04+** 服务器，装好 **Hermes Agent v0.16+（v0.17 已验证兼容）+ Docker**，并配好模型供应商凭证（GLM / DeepSeek 或 OpenRouter）。当前注册表为内存实现，**PostgreSQL / Redis 属阶段 13 生产化**，MVP 不必预装。
...
| GET | `/api/projects/{id}/deliverable` | 交付总览：is_done（阶段 complete + 发布清单 + QA 放行 + 完整性）+ 运行方式 |
| GET | `/api/projects/{id}/events` | SSE 事件流（当前为一次性看板快照，持续推送属阶段 13） |
| GET | `/api/projects` | 列出所有项目（阶段 + 任务统计），Web UI 用 |
| GET | `/api/projects/{id}/state` | 项目状态详情（阶段 / 设计文档 / QA 状态） |
| GET | `/api/projects/{id}/artifact-content` | 读产物文件（白名单目录 + 防穿越，512KB 上限） |
| GET | `/api/projects/{id}/conversation` | 对话历史（读平台自有 JSONL，不依赖 Hermes 内部库） |
| GET | `/` | 单文件 Web UI（`webui.html`，带 CSP 安全头；缺文件返回 404） |

**安全红线**：Hermes 的 `/v1/*` 与 dashboard 绝不直接对外（直连等于暴露 terminal/file/web 全工具），
**始终只绑本机**；对外鉴权全在本网关（X-Token）。

**局域网访问 CEO 交互页**：控制平面默认只绑 `127.0.0.1`。要让局域网其它机器访问，部署时设
`PLATFORM_BIND_HOST=0.0.0.0`（`make run` 同名变量），然后浏览器开 `http://<本机局域网IP>:9000/` 填
PLATFORM_TOKEN。⚠️ 控制平面仅 X-Token 鉴权、**无 TLS**，只适合**可信局域网**；非本机地址却仍用默认
token `change-me` 会被**拒绝启动**。跨不可信网络/公网须前置 nginx/Caddy 做 TLS。

## 技术栈

Hermes Agent v0.16+（v0.17 验证兼容）· Kanban multi-agent · FastAPI · Docker ·（PostgreSQL/Redis 阶段 13）
